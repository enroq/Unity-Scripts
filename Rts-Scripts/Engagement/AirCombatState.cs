...
public class AirCombatState : KaryonBehaviour, ICombatant
{
    [SerializeField]
    ...
    {
        get { return CombatantTargetType.Air; }
    }
    ...
    internal void QueryPursuitStatus()
    {
        ...
        if (!IsInStopRange())
        {
            if (!m_NavigatingToTarget)
            {
                ...
                ((BaseUnit)m_EntityRelative).GoToPosition
                    (m_TargetPositionCache, m_AttackRange - (m_AttackStopBuffer * Mathf.PI));

            }
            else if(m_TargetPositionCache != m_CurrentTarget.transform.position)
            {
                m_TargetPositionCache = m_CurrentTarget.transform.position;
        ...
        {
            m_NavigatingToTarget = false; ((BaseUnit)m_EntityRelative).HaltNavigation();
        }
    }
